...
directory = 'data/trajectory_velocity/val_smooth/' # Replace with the path to your directory
t0 = time.time()
num_tested = 0
fde = []
ade = []
...
for filename in os.listdir(directory):
    ...
    for i in range(0, len(past) - 1):
          line = past[i].strip().split('\t')
          line = [float(j) for j in line[1:]]
          pastarray.append(line)
    #print(pastarray)
    paststr = '\n'.join(past)
    paststr = paststr + '\n' + "velocity"
    print(paststr)
    ...
    #print(gtarray)
    prediction = gtarray.copy()
    start_ids = encode(paststr)
        ...
        with ctx:
            for k in range(num_samples):
                for i in range(10):
                    ...
                    last = pastarray[-1]
                    newp = [round(velocity[0] + last[0],4) , round(velocity[1] + last[1],4), round(velocity[2] + last[2],4) ]
                    pastarray.append(newp)
                    prediction[i][0] = newp[0]
                    prediction[i][1] = newp[1]
                    prediction[i][2] = newp[2]
                    past = past[1:]

                    newpast = 'past'  + '\t' + str(newp[0]) + '\t' + str(newp[1]) + '\t' + str(newp[2])
                    past.append(newpast)
                    paststr = '\n'.join(past)
                    paststr = paststr + '\n' + "velocity"
                    #print(paststr)
    ...
    for i in range(len(dx)):
        ...
        #if abs(dx[i]) > 0.05 or abs(dy[i]) > 0.05 or abs(dz[i]) > 0.05:
    ...
    if p[9] < 100:
        fde.append(p[9])
print('number of test' + str(num_tested))
print('number of test point' + str(len(ade)))
